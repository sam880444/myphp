...
if (isset($_GET['totalRows'])) {
  $totalRows = $_GET['totalRows'];
	...
	$pdoStmt = $pdo->prepare($query_all_records_count);
	$pdoStmt->execute();
	// 再取出查詢結果內的紀錄筆數，放到變數 $totalRows內
	$totalRows  = $pdoStmt->fetchColumn(0);
}
...
<table  class="table_color" width="720" border="2" align="center" cellpadding="2" cellspacing="2" >
  ...
  <?php 
     ...
     $arr2D = $result->fetchAll(PDO::FETCH_ASSOC);
     foreach($arr2D as $row){ ?>
       <tr>  
         <td><?php echo $row['bookID']; ?></td> 
         <td><a href="BookUpdate.php?bookID=<?php echo $row['bookID']; ?>"><?php echo $row['title']; ?></a></td>
